  ...
  return (
    <div className="space-y-12 pb-8 sm:space-y-16">
      <section className="page-shell">
        <div className="glass-panel surface-outline relative overflow-hidden rounded-[36px] border border-white/60 px-6 py-8 sm:px-10 sm:py-12">
          <div
          ...
          <div className="relative grid gap-10 lg:grid-cols-[minmax(0,1.25fr)_360px] lg:items-center">
            <div>
              ...
              <h1 className="section-title brand-gradient-text">{profile.title}</h1>
              <p className="mt-3 text-xl font-medium text-slate-700 sm:text-2xl">{profile.subtitle}</p>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link
                  href="/contato"
                  className="inline-flex items-center justify-center rounded-full border border-[rgba(9,66,125,0.16)] bg-white/80 px-5 py-3 text-sm font-semibold text-slate-700 hover:border-[rgba(9,66,125,0.26)] hover:text-slate-950"
                >
                  Falar com o DADG
                </Link>
              </div>
            ...
            <div className="mx-auto w-full max-w-[320px]">
              <div className="relative rounded-[32px] border border-white/60 bg-white/78 p-4 shadow-[0_24px_60px_rgba(7,48,89,0.12)]">
                <div
                  className="absolute inset-x-10 top-0 h-32 rounded-full blur-3xl"
                  style={{ backgroundColor: profile.accent.surface }}
                />
                <div className="relative aspect-square overflow-hidden rounded-[28px] border border-white/70 bg-white">
                  <Image
      ...
      <section className="page-shell grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
        <InfoCard title="Missao" description={profile.mission} className="h-full">
        ...
        <InfoCard title="Valores que orientam a coordenadoria" className="h-full">
          <ul className="space-y-3">
            {profile.values.map((value) => (
              <li key={value} className="flex items-start gap-3 rounded-[20px] bg-white/70 px-4 py-3">
                <CheckCircle2
                  className="mt-0.5 h-5 w-5 flex-none"
                  style={{ color: profile.accent.primary }}
                />
                <span className="text-sm font-medium text-slate-700 sm:text-base">{value}</span>
              </li>
            ))}
          </ul>
        </InfoCard>
      ...
      <section className="page-shell space-y-8">
        <SectionHeading
          eyebrow="Destaques"
          title={`Como a ${profile.shortName} agrega valor ao ecossistema academico`}
          description="Cada coordenadoria tem um papel proprio, mas todas compartilham o objetivo de fortalecer a experiencia estudantil de forma consistente."
        />
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {profile.highlights.map((highlight) => (
            <InfoCard
              ...
              description="Um ponto central da coordenadoria dentro da atuacao do DADG e da experiencia academica dos estudantes."
            />
          ))}
        </div>
      </section>
      ...
      {profile.team?.length ? (
        <section className="page-shell space-y-8">
          <SectionHeading
            ...
            description="A coordenadoria tambem ganha uma apresentacao mais humana, aproximando estudantes de quem faz o trabalho acontecer."
          />
          ...
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {profile.team.map((member) => (
              <article
                key={member.name}
                className="glass-panel surface-outline overflow-hidden rounded-[28px] border border-white/70 p-4"
              >
                ...
                <div className="px-2 pb-2 pt-5">
                  ...
                  <p className="mt-2 text-sm uppercase tracking-[0.28em] text-slate-500">{member.role}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      {profile.qrCode ? (
        <section className="page-shell">
          <div className="glass-panel surface-outline overflow-hidden rounded-[32px] border border-white/70 p-6 sm:p-8">
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-center">
              ...
              <div className="mx-auto w-full max-w-[260px] rounded-[28px] bg-white p-4 shadow-[0_24px_60px_rgba(7,48,89,0.12)]">
